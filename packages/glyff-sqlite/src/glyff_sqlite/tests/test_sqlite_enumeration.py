...
async def test_committed_rows_are_pulled_in_batches(tmp_path: Path):
    ...
    backend = SQLiteBackend(tmp_path / "batched.sqlite3", session_id="batched")
    ...
    seen = 0
    async for _ in backend.repository.executions():
        seen += 1
    assert seen == count
...
async def test_a_partially_consumed_enumeration_closes_its_connection(
    ...
    backend = SQLiteBackend(tmp_path / "partial.sqlite3", session_id="partial")
    async with TransactionScope(backend.transaction_provider):
    ...
    enumeration = cast(AsyncGenerator[Execution, None], backend.repository.executions())
