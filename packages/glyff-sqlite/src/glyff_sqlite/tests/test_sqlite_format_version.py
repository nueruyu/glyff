...
class TestConfigurableTablePrefix:
    def test_default_prefix_derives_both_table_names(self, tmp_path: Path):
        ...
        assert client._table_name == "glyff_executions"  # pyright: ignore[reportPrivateUsage]
        assert client._session_domains_table_name == (  # pyright: ignore[reportPrivateUsage]
            "glyff_session_domains"
        )
        assert client._meta_table_name == "glyff_meta"  # pyright: ignore[reportPrivateUsage]
