...
class SessionMigrator(Protocol):
    ...
    with a side effect happens outside the transaction that would undo it.
...
class SessionMigration(Protocol):
...
class MigratableBackend(Protocol):
